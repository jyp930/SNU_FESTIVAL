...
function TypingTextTransition({ text }) {
  ...
  return (
    <S.StyledTypingTextTransition id="StyledTypingTextTransition">
      <S.Offscreentext id="offscreen-text" />
      <S.Text id="text" />
      <S.Svg id="svg" />
    </S.StyledTypingTextTransition>
  );
}
export default TypingTextTransition;
...
function TextAnimationTrigger(Text) {
  ...
  const styledTypingTextTransition = document.getElementById('StyledTypingTextTransition');
  const svg = selectSVG('svg');
  const text = document.getElementById('text');
  const offscreenText = document.getElementById('offscreen-text');
  const inputText = [];
  let width = styledTypingTextTransition.offsetWidth;
  let height = styledTypingTextTransition.offsetHeight;
  let textSize = 0;
  let textCenter = 0;
  const letters = [];
  ...
  const animateLetterIn = letter => {
    ...
    TweenLite.to(letter, 0.2, { rotation, ease: Power3.easeInOut });
