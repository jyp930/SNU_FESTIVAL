export function getPasswordFromEmail(email, password = 1, length = 5) {
  if (!email) return 'KJHSRH';
  ...
  return passwordNumbers.map(number => String.fromCharCode((number % 26) + 65)).join('').slice(0, length);
}
